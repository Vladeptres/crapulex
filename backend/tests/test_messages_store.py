...
def store() -> MessagesStore:
    with patch("core.messages_store.MongoClient"):
...
def test_add_message_validates_and_inserts(store):
    ...
    with (
        ...
        patch("core.messages_store.Message.model_validate") as mock_validate,
    ):
...
def test_get_messages_returns_validated(store):
    ...
    with (
        patch.object(store, "messages_collection") as mock_coll,
        patch("core.messages_store.Message.model_validate", side_effect=lambda x: x),
    ):
...
def test_add_react_success(store: MessagesStore):
    ...
    with patch.object(store, "messages_collection") as mock_coll:
        ...
        with patch("core.messages_store.logger") as mock_logger:
...
def test_get_reacts(store: MessagesStore):
    ...
    with (
        patch.object(store, "messages_collection") as mock_coll,
        patch("core.messages_store.React.model_validate", side_effect=lambda x: x),
    ):
